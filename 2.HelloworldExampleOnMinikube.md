...
$ kubectl get configuration -n myproject -o yaml
apiVersion: v1
items:
...
$ kubectl get revision -n myproject -o yaml
apiVersion: v1
items:
- apiVersion: serving.knative.dev/v1alpha1
  kind: Revision
...
$ kubectl get route helloworld -n myproject -o yaml
apiVersion: serving.knative.dev/v1alpha1
kind: Route
metadata:
  clusterName: ""
...
```
$ export URL=$(kubectl get route helloworld -n myproject -o jsonpath={.status.domain})
...
* Create the `IP_ADDRESS` environment variable to easily access your application over HTTP
